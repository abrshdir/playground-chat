...
// Handle errors from the chat request 
...
// Fetch available models and return a structured object
async function getOllamaModel() {
	...
	try {
		...
		// Find your specific model instead of using arbitrary index
		const targetModel = data.models.find(m => m.name.includes('qwen2.5-coder'));
		...
		return {
			...
			countTokens: (
				...
				try {
					...
					// Handle LanguageModelChatMessage input
					else {
						// Convert message content parts to a single string
						content = text.content
							.map(part => typeof part === "string" ? part : JSON.stringify(part))
							.join(" ");
					}

					// Token counting logic (simple whitespace-based splitting)
					const tokens = content.split(/\s+/).length;
					return Promise.resolve(tokens);
				} catch (error) {
					console.error("Error counting tokens:", error);
					return Promise.resolve(0); // Fallback value
				}
			},
			...
			async sendRequest(
				...
				const chatUrl = "http://localhost:11434/api/generate";

				// Convert messages to prompt (adjust based on your model's requirements)
				const doloresPrompt = messages
				...
				const textStream = async function* () {
					...
					if (buffer.trim()) {
						try {
							...
							console.error('Error parsing final JSON chunk:', buffer, err);
						}
					}
				};
